...
- Create the Secret with `--from-file` as shown to avoid embedding plaintext in
  manifests. The Secret will mount a file at `/run/secrets/runtime_config`
  containing the runtime configuration JSON object used by the container's entrypoint.
